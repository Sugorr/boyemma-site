...
export default function OurPhilosphy () {
    ...
    const textView = useInView(textRef, {margin: "120px 0px 0px 0px", amount: "all"})
    const missionView = useInView(missionRef, {margin: "120px 0px 250px 0px", amount: "all"})
    const visionView = useInView(visionRef, {margin: "120px 0px 250px 0px", amount: "all"})
    const storyView = useInView(storyRef, {margin: "120px 0px 250px 0px", amount: "all"})
    ...
    return (
        <section className="flex flex-col justify-between items-center w-full gap-40 py-6 mt-12">
            ...
            <section className="flex flex-col gap-36">
                {/* Mission */}
                <section ref={missionRef} className="flex w-full">
                    {/* Content */}
                    <motion.section
                    initial={{ 
                        opacity: 0,
                    ...
                    className="relative flex items-center justify-center w-full px-12">
                        <div>
                            <img className="h-[420px] w-full rounded-xl object-cover object-center" src={MissionImg} alt="" />
                        </div>
                    </motion.div>
                </section>

                {/* Vision */}
                <section ref={visionRef} className="flex w-full">
                    {/* Image */}
                    <motion.div
                    initial={{ 
                        opacity: 0,
                    ...
                    className="relative flex items-center justify-center w-full px-12">
                        <div>
                            <img className="h-[420px] w-full rounded-xl object-cover object-center" src={VisionImg} alt="" />
                        </div>
                    </motion.div>
                    {/* Content */}
                    <motion.section
                ...
                {/* Our Story */}
                <section ref={storyRef} className="flex w-full">
                    {/* Content */}
                    <motion.section
                    initial={{ 
                        opacity: 0,
                    ...
                    animate={{ 
                        opacity: storyView ? 1 : 0,
                        y: storyView ? 0 : 24,
                        transition: {
                            duration: 0.55,
                            ease: "easeOut",
                    }}}
                    className="flex flex-col justify-start items-start gap-12 w-full px-12">
                        <div className="flex items-center gap-4 text-4xl">
                            <div className="p-2 h-12 w-12 bg-default-accent rounded-full">
                        ...
                        <section>
                            From humble beginnings, Boyemma Trucking Services (Formertly Boyemma Trading) started at the year 2012 with only 2 ten-wheeler trucks with their garage located on San Leonardo, Nueva Ecija. A great business owned, founded, and managed by Emmaruth Cooper which had a great experience in the trucking industry.
                        </section>
                    ...
                    className="relative flex items-center justify-center w-full px-12">
                        <div>
                            <img className="h-[420px] w-full rounded-xl object-cover object-center" src={StoryImg} alt="" />
                        </div>
                    </motion.div>
                </section>
            </section>
